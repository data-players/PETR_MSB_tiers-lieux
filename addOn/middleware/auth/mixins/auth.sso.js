...
const redirectToRemoteLogout = require('../middlewares/redirectToRemoteLogout');
const localLogout = require('../middlewares/localLogout');
const addInfoToReq = require('../middlewares/addInfoToReq');
...
const AuthSSOMixin = {
  ...
  actions: {
    async loginOrSignup(ctx) {
      const { ssoData, tokenSet } = ctx.params;
      ...
      const token = await ctx.call('auth.jwt.generateToken', { payload: { webId , idToken:tokenSet.id_token} });
    ...
    getApiRoutes() {
      ...
      return [
        ...
        },
        {
          ...
          use: [sessionMiddleware, this.passport.initialize(), this.passport.session()],
          aliases: {
            'GET /': [saveRedirectUrl, localLogout, addInfoToReq({strategy: this.strategy}), redirectToRemoteLogout]
          }
        }
      ];
    }
